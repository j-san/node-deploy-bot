...
Shell.prototype.run = function () {
    ...
    for (var i in args) {
        ...
        if (deploy.tasks[name].profile) {
            // already initialized, resolve callback
            promises.push(deploy.tasks[name].profile);
        }else {
            var init = deploy.tasks[name].init;
    ...
    return Q.all(promises).then(function (results) {
        for (var i in results) {
            ...
            var name = args[i];
            if (!('profile' in deploy.tasks[name])) {
                deploy.tasks[name].profile = profile;
            }
        }
